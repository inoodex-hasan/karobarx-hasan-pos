$(document).ready(function() {
    var is_ai_template = window.location.pathname.split('/')[1] === 'ai-template';
    var stock_transfer_base_url = is_ai_template ? '/ai-template' : '';
    ...
    if ($.fn.DataTable.isDataTable('#stock_transfer_table')) {
        $('#stock_transfer_table').DataTable().destroy();
        $('#stock_transfer_table tbody').empty();
    }
    ...
    stock_transfer_table = $('#stock_transfer_table').DataTable({
        ...
        fixedHeader:false,
        aaSorting: [[0, 'desc']],
        ajax: stock_transfer_base_url + '/stock-transfers',
        dom:
        ...
        initComplete: function() {
            var relocate = function() {
                var $wrapper = $('#stock_transfer_table_wrapper');
                ...
                if ($length.length) $('#stock_transfer_dt_length').empty().append($length);
                if ($filter.length) $('#stock_transfer_dt_filter').empty().append($filter);
                if ($info.length) $('#stock_transfer_dt_info').empty().append($info);
                if ($paginate.length) $('#stock_transfer_dt_paginate').empty().append($paginate);
            };
...
function get_stock_transfer_details(rowData) {
    ...
    var div = $('<div/>')
        ...
        .text('Loading...');
    $.ajax({
        url: stock_transfer_base_url + '/stock-transfers/' + rowData.DT_RowId,
        dataType: 'html',
