$(document).ready(function() {
    ...
    $('#supplier_id').select2({
        ajax: {
            url: purchase_base_url + '/purchases/get_suppliers',
            dataType: 'json',
            delay: 250,
        ...
        $('#search_product')
            .autocomplete({
                source: function(request, response) {
                    $.getJSON(
                        purchase_base_url + '/purchases/get_products',
    ...
    var purchase_base_url = window.location && window.location.pathname && window.location.pathname.indexOf('/ai-template') === 0
        ? '/ai-template'
        : '';

    var is_ai_template_purchase = window.location && window.location.pathname && window.location.pathname.indexOf('/ai-template') === 0;

    //Purchase table - only initialize if table element exists
    if ($('#purchase_table').length > 0) {
        ...
        purchase_table = $('#purchase_table').DataTable({
            ...
            scrollX: true,
            scrollCollapse: true,
            pageLength: is_ai_template_purchase ? 25 : 10,
            lengthMenu: is_ai_template_purchase
                ? [[10, 25, 50, 100, -1], [10, 25, 50, 100, 'All']]
                : [[10, 25, 50, 100, -1], [10, 25, 50, 100, 'All']],
            dom: is_ai_template_purchase
                ? "<'row'<'col-sm-12 col-md-6'l><'col-sm-12 col-md-6 text-md-end'f>>" +
                  "<'row'<'col-sm-12'tr>>" +
                  "<'row'<'col-sm-12 col-md-5'i><'col-sm-12 col-md-7 text-md-end'p>>"
                : "<'row'<'col-sm-12 col-md-6'l><'col-sm-12 col-md-6'r>>" +
                  "<'row'<'col-sm-12't>>" +
                  "<'row'<'col-sm-12 col-md-6'i><'col-sm-12 col-md-6'p>>",
            ajax: {
                url: purchase_base_url + '/purchases',
                data: function(d) {
            ...
            fnDrawCallback: function(oSettings) {
                ...
                if (is_ai_template_purchase) {
                    var $wrapper = $('#purchase_table_wrapper');
                    $('#purchase_dt_length').html($wrapper.find('.dataTables_length'));
                    $('#purchase_dt_filter').html($wrapper.find('.dataTables_filter'));
                    $('#purchase_dt_info').html($wrapper.find('.dataTables_info'));
                    $('#purchase_dt_paginate').html($wrapper.find('.dataTables_paginate'));
                }
            },
...
function get_purchase_entry_row(product_id, variation_id) {
    if (product_id) {
        ...
        $.ajax({
            method: 'POST',
            url: purchase_base_url + '/purchases/get_purchase_entry_row',
            dataType: 'html',
            data: data,
            success: function(result) {
...
$(document).on('click', 'button#submit_purchase_form', function(e) {
    ...
    $('form#add_purchase_form').validate({
        rules: {
            ref_no: {
                remote: {
                    url: purchase_base_url + '/purchases/check_ref_number',
                    type: 'post',
                    data: {
                        ref_no: function() {
                            return $('#ref_no').val();
