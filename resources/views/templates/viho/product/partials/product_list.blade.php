            ...
            <td colspan="{{$colspan}}">
                <div class="d-flex flex-wrap gap-2">
                    @can('product.delete')
                        ...
                        {!! Form::submit(__('lang_v1.delete_selected'), ['class' => 'btn btn-sm btn-danger', 'id' => 'delete-selected']) !!}
                        {!! Form::close() !!}
                    @endcan
                    ...
                    @can('product.update')
                        @if(config('constants.enable_product_bulk_edit'))
                            ...
                            <button type="submit" class="btn btn-sm btn-primary" id="edit-selected">
                                <i class="fa fa-edit"></i> {{__('lang_v1.bulk_edit')}}
                            </button>
                            {!! Form::close() !!}
                        @endif

                        <button type="button" class="btn btn-sm btn-accent update_product_location" data-type="add">
                            @lang('lang_v1.add_to_location')
                        </button>

                        <button type="button" class="btn btn-sm btn-neutral update_product_location" data-type="remove">
                    ...
                    {!! Form::submit(__('lang_v1.deactivate_selected'), ['class' => 'btn btn-sm btn-warning', 'id' => 'deactivate-selected']) !!}
                    {!! Form::close() !!}
